...
from app.timesheet_line.infra.external.odoo.odoo_timesheet_gateway import OdooTimesheetLineGateway
...
def _task_to_dict(task):
    ...
    return {
        ...
        "name": task.name,
        "state": task.state,
        "subtasks": [_task_to_dict(subtask) for subtask in task.subtask] if task.subtask else []
    }
...
def get_all_tasks_in_project(project_id: int) -> str:
    ...
    try:
        ...
        use_case = ObtenerTareasUseCase(task_gateway, OdooTaskGateway(odoo_connection), OdooEmployeeGateway(odoo_connection))
...
def _parse_date(date_str: str) -> date:
    ...
    try:
        ...
        raise ValueError(f"Formato de fecha inválido: '{date_str}'. Usa YYYY-MM-DD (ej: 2024-11-14)")
...
@tool
def create_timesheet_entry(runtime: ToolRuntime[Context], project_id: int, task_id: int, hours: float, date_str: str, description: str = "") -> str:
...
@tool
def create_multiple_timesheet_entries(runtime: ToolRuntime[Context], entries_json: str) -> str:
    ...
    try:
        ...
        for idx, entry in enumerate(entries_data):
            try:
                ...
                if not all(k in entry for k in ["project_id", "task_id", "hours", "date_str"]):
                    return json.dumps({
                        "success": False,
        ...
        return json.dumps({
            "success": False,
            "error": "Error al crear timesheets",
            "message": str(e)
        }, ensure_ascii=False)
        ...
        return json.dumps({
            "success": False,
            "error": "Timesheets no encontrados",
            "message": str(e)
        }, ensure_ascii=False)
