...
class OdooDashboardDataGateway(DashboardDataGateway):
    ...
    def get_team_timesheet_data(
        ...
        try:
            ...
            print("respuesta de odoo:", odoo_timesheet_lines)
...
# Script de prueba
def main():
    """Función principal para probar el gateway de dashboard."""
    try:
        print("🔄 Conectando a Odoo...")
        
        # Obtener conexión a Odoo
        ...
        gateway = OdooDashboardDataGateway(odoo_connection)
        
        # Definir parámetros de prueba
        user_id = 1  # ID del usuario para probar (ajusta según tu Odoo)
        
        # Probar período: enero 2025 (ajusta según tengas datos)
        from datetime import date
        date_from = date(2025, 9, 1)
        date_to = date(2025, 9, 9)
        
        print(f"📊 Probando get_team_timesheet_data...")
        print(f"   Usuario ID: {user_id}")
        print(f"   Período: {date_from} a {date_to}")
        
        # Obtener datos de timesheet del equipo
        timesheet_data = gateway.get_team_timesheet_data(user_id, date_from, date_to)
        
        print(f"📈 Resultados de timesheet:")
        print(f"   Total registros encontrados: {len(timesheet_data)}")
        
        if timesheet_data:
            print(f"   Primeros 3 registros:")
            for i, record in enumerate(timesheet_data[:3]):
                print(f"     {i+1}. {record.project.name} - {record.hours}h - {record.date}")
        
        print(f"\n👥 Probando get_active_team_users_count...")
        users_count = gateway.get_active_team_users_count(user_id)
        print(f"   Usuarios activos en el equipo: {users_count}")
        
        print("\n✅ Pruebas completadas exitosamente!")
        
        return timesheet_data, users_count
