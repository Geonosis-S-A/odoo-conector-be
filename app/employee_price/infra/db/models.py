...
class EmployeePriceModel(SQLModel, table=True):
    ...
    cost_per_hour: float = Field(
        gt=0, description="Costo por hora del empleado (debe ser mayor a 0)"
    )
...
@event.listens_for(EmployeePriceModel, "before_update")
def update_timestamp(mapper, connection, target):
    """Actualiza automáticamente el timestamp de updated_at antes de cada actualización"""
