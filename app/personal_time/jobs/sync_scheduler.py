    ...
    try:
        ...
        run_details = RunDetails(
            new_requests=NewRequestsPhaseDetail(
                total_processed=new_requests_result.total_processed
                if new_requests_result
                else 0,
                successfully_synced=new_requests_result.successfully_synced
                if new_requests_result
                else 0,
                skipped=new_requests_result.skipped_count if new_requests_result else 0,
                errors_count=new_requests_result.errors_count
                if new_requests_result
                else 0,
                synced_records=[
                    SyncedRecordDetail(**d)
                    for d in (
                        new_requests_result.synced_details
                        if new_requests_result
                        else []
                    )
                ],
                skipped_records=[
                    SkippedRecordDetail(**d)
                    for d in (
                        new_requests_result.skipped_details
                        if new_requests_result
                        else []
                    )
                ],
                errors=[
                    ErrorRecordDetail(**d)
                    for d in (
                        new_requests_result.error_details if new_requests_result else []
                    )
                ],
            ),
            status_updates=StatusUpdatesPhaseDetail(
                total_processed=status_updates_result.total_processed
                if status_updates_result
                else 0,
                status_updates_count=status_updates_result.status_updates_count
                if status_updates_result
                else 0,
                skipped=status_updates_result.skipped_count
                if status_updates_result
                else 0,
                errors_count=status_updates_result.errors_count
                if status_updates_result
                else 0,
                updated_records=[
                    StatusUpdateRecordDetail(**d)
                    for d in (
                        status_updates_result.status_update_details
                        if status_updates_result
                        else []
                    )
                ],
                skipped_records=[
                    SkippedRecordDetail(**d)
                    for d in (
                        status_updates_result.skipped_details
                        if status_updates_result
                        else []
                    )
                ],
                errors=[
                    ErrorRecordDetail(**d)
                    for d in (
                        status_updates_result.error_details
                        if status_updates_result
                        else []
                    )
                ],
            ...
            summary=SyncSummary(
                total_processed=(
                    ...
                    + (
                        status_updates_result.total_processed
                        if status_updates_result
                        else 0
                    )
                ),
                total_synced=(
                    (
                        new_requests_result.successfully_synced
                        if new_requests_result
                        else 0
                    )
                    + (
                        status_updates_result.status_updates_count
                        if status_updates_result
                        else 0
                    )
                ),
...
def start_scheduler():
    ...
    scheduler.add_job(
        ...
        trigger=CronTrigger(hour=18, minute=00, timezone=ARGENTINA_TZ),
